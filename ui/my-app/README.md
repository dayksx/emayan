...
**Petty Ledger (Emayan):** on-chain grievance recording, XRP custody, and withdrawal rules live in the **`petty_ledger`** contract ([`contracts/petty-ledger/`](../../contracts/petty-ledger/README.md)). The UI should target that memo format and the deployed **vault** account address when wiring stake-based flows. Built with Turborepo, inspired by Scaffold-ETH-2.
